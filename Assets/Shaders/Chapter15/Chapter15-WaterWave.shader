	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				float2 displacement = _Time.y * float2(_WaveXSpeed, _WaveYSpeed);

				// Get the normal in tangent space.
				fixed3 bump1 = UnpackNormal(tex2D(_WaveMap, i.uv.zw + displacement)).rgb;
				fixed3 bump2 = UnpackNormal(tex2D(_WaveMap, i.uv.zw - displacement)).rgb;
				fixed3 bump = normalize(bump1 + bump2);

				// Compute the offset in tangent space.
				// ??? 在 Unity 2017.4.37c2 (64-bit) 无法达到想要的折射效果, 需要增大 _Distortion 的上限.
				...
				fixed4 texColor = tex2D(_MainTex, i.uv.xy + displacement);
